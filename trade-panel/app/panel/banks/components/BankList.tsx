...
import { BankEditSheet } from './BankEditSheet';

function rowSearchText(row: BankRow) {
  return [row.name, row.isActive ? 'aktif' : 'pasif'].join(' ').toLowerCase();
}

export function BankList() {
  const { user } = useAuth();
  const canWrite = canAccess(user, PERMS.BANKS_WRITE);
  ...
  const load = useCallback((silent = false) => {
    ...
    fetchBanks(businessId || undefined)
      .then(setRows)
      .catch((e) => {
            ...
            <table className="w-full min-w-[640px] text-left">
              ...
              <tbody>
                {filtered.map((row) => (
                  ...
                  >
                    <td className={tdClass}>{row.businessName ?? '—'}</td>
                    ...
                    <td className={tdClass}>{row.accountCount}</td>
                    <td className={tdClass}>
                      {row.isActive ? (
