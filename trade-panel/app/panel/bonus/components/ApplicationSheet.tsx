...
const STATUS_LABEL: Record<CampaignApplicationStatus, string> = {
...
export function ApplicationSheet({ id, canWrite, onClose, onSaved }: Props) {
  const [row, setRow] = useState<CampaignApplicationRow | null>(null);
  ...
  async function onSaveAmount() {
    ...
    setError(null);
    try {
      const updated = await updateCampaignApplication(id, { amount: value });
      setRow(updated);
  ...
  async function onApprove() {
    ...
    const value = Number(amountInput.replace(',', '.'));
    if (!Number.isFinite(value) || value <= 0) {
      setError('Onay için geçerli bir tutar girin');
      return;
    }
    setSubmitting(true);
    setError(null);
    try {
      await updateCampaignApplication(id, { amount: value, status: 'approved' });
      onSaved();
      onClose();
    } catch (err) {
  ...
  async function onReject() {
    ...
    setError(null);
    try {
      await updateCampaignApplication(id, { status: 'rejected' });
      onSaved();
      onClose();
    } catch (err) {
  ...
  async function onDelete() {
    if (!canWrite || !confirm('Bu başvuruyu silmek istediğinize emin misiniz?'))
      return;
    setSubmitting(true);
    try {
      await deleteCampaignApplication(id);
      onSaved();
      onClose();
    } catch (err) {
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 flex max-h-[92vh] flex-col rounded-t-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
        <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-3 dark:border-zinc-800">
          <h2 className="text-base font-semibold">Kampanya başvurusu</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          {loading || !row ? (
            ...
            <>
              {row.campaign.imageUrl && (
                <img
                  src={row.campaign.imageUrl}
                  alt=""
                  className="h-32 w-full rounded-lg object-cover"
                />
              )}
              <p className="text-sm font-semibold">{row.campaign.title}</p>
              ...
              <p className="text-xs text-zinc-500">
                Başvuru: {formatDate(row.createdAt)}
              </p>

              <div className="rounded-lg border border-zinc-200 px-3 py-2.5 dark:border-zinc-700">
                <p className="mb-1 text-sm font-medium">Kampanya tutarı</p>
                {canWrite && !settled ? (
