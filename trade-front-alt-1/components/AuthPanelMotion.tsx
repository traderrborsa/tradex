...
function MiniSparkline({ values, up, compact }: { values: number[]; up: boolean; compact?: boolean }) {
  ...
  return (
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" aria-hidden>
      <polygon points={areaPoints} fill={up ? 'rgba(229,37,32,0.12)' : 'rgba(113,113,122,0.1)'} />
...
function FloatingCard({ item, compact }: { item: FloatingItem; compact?: boolean }) {
  return (
    <div
      className={`auth-float-card rounded-md p-3 ${compact ? 'w-[148px]' : 'w-[176px]'} ${compact ? 'p-2.5' : 'p-3.5'}`}
    >
      <div className="flex items-start justify-between gap-1.5">
        <div className="min-w-0">
          <p className={`truncate font-bold text-foreground ${compact ? 'text-xs' : 'text-sm'}`}>{item.symbol}</p>
          {!compact && <p className="truncate text-[11px] text-muted">{item.label}</p>}
        </div>
        <span
      ...
      <p className={`mt-1.5 font-bold tabular-nums text-foreground ${compact ? 'text-sm' : 'text-base'}`}>{item.price}</p>
