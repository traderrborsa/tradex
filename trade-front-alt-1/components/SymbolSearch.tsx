...
export function SymbolSearch({
  ...
  const shellClass = compact
    ? 'rounded-full bg-input py-1 pl-5 pr-1'
    : 'rounded-full bg-input py-1.5 pl-5 pr-1.5';
    ...
    <div ref={rootRef} className="relative w-full">
      <div className={`flex items-center gap-2 ${shellClass}`}>
        <input
          ...
          placeholder="Hisse, döviz veya kripto ara"
        ...
        <button
          ...
          disabled={submitting}
          aria-label="Ara"
          className={`flex ${btnSize} shrink-0 cursor-pointer items-center justify-center rounded-full bg-accent text-accent-fg transition hover:bg-[var(--accent-hover)] disabled:cursor-wait disabled:opacity-80`}
        >
          <SearchIcon />
        </button>
      </div>
